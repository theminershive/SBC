...
//
// Bootup the masternode, look for a 7500 SBC input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* rewards are not supported in sbc.conf */
...
{
    ZspankbankAddress address(collateralAddress);
