...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: sbc.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: sbcd.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 28452)") + "\n";
    ...
    strUsage += "  -synctime              " + _("Sync time with other nodes. Disable if time on your system is lucid e.g. syncing with NTP (default: 1)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 18452)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizesbcamount=<n> " + _("Keep N SBC anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. SBC is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. SBC is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("SBC version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "SBC server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of SBC") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart SBC to complete") << "\n";
    ...
    nAnonymizeSBCAmount = GetArg("-anonymizesbcamount", 0);
    if(nAnonymizeSBCAmount > 999999) nAnonymizeSBCAmount = 999999;
    if(nAnonymizeSBCAmount < 2) nAnonymizeSBCAmount = 2;
    ...
    LogPrintf("Anonymize SBC Amount %d\n", nAnonymizeSBCAmount);
