...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeSBCAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeSBCAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeSBCAmount = strAnonymizeSBCAmount.remove(strAnonymizeSBCAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeSBCAmount + " SBC / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeSBCAmount*COIN) nMaxToAnonymize = nAnonymizeSBCAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeSBCAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeSBCAmount));
        strAnonymizeSBCAmount = strAnonymizeSBCAmount.remove(strAnonymizeSBCAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeSBCAmount + " SBC / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
        ...
        strAmountAndRounds = "<span style='color:red;'>" +
                ...
                " SBC / " + tr("%n Rounds", "", nDarksendRounds) + "</span>";
    }
    ui->labelAmountRounds->setText(strAmountAndRounds);
