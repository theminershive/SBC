...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeSBCAmount"))
        settings.setValue("nAnonymizeSBCAmount", 1000);
    nAnonymizeSBCAmount = settings.value("nAnonymizeSBCAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeSBCAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeSBCAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeSBCAmount:
            return QVariant(nAnonymizeSBCAmount);
        default:
            return QVariant();
        }
    }
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeSBCAmount:
            nAnonymizeSBCAmount = value.toInt();
            settings.setValue("nAnonymizeSBCAmount", nAnonymizeSBCAmount);
            emit AnonymizeSBCAmountChanged(nAnonymizeSBCAmount);
            break;
        default:
            break;
        }
